			...
			for (int j = 0; j < n; j++) { // x
				switch (type) {
					...
					case 1:
						...
						break;
					case 2:// 1-cloud
						node = new NodeCloud(id, "C" + String.valueOf(id), j * space, i * space, Constants.RANGE[type],
								Constants.RES[type]);
					default:
		...
		for (NodeVehicle n1 : root.getNodeNeighbor().get(i)) { // node neighbor tại thời điểm i
			if (root.getNodeNeighbor().get(i + 1).contains(n1)) { // n1 is still neigbours in next ts --> set lvl = 1
				...
				if (!listNodeReq.contains(n1)) { // nếu không có n1 ở listNodeReq ở hiện tại --> thêm vào rtable, set
													// npath = 1
					rtable.add(id, new RTable(id, n1, root.getName(), 1, n1.getRes(), req));
				...
				if (n.getLvl() == (maxHop - 1)) {
					for (NodeVehicle n1 : n.getNodeNeighbor().get(i)) {
						...
						if (n1.getId() != root.getId() && (n1.getLvl() > n.getLvl()) && (!listNodeReq.contains(n1))) {
							n1.setLvl(maxHop);
							rtable.add(id, new RTable(id, n1, n.getName(), maxHop, n1.getRes(), req));
